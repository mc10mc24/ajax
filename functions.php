...
function wpshout_register_taxonomy() {
    ...
    register_taxonomy( 'awesome_taxonomy', array( 'game-reviews' ), $args );
}

//
add_action('wp_enqueue_scripts', 'ajax_select_data');
function ajax_select_data(){
	wp_enqueue_script( 'jquery.min', get_template_directory_uri() . '/js/jquery.min.js', array('jquery') );
    wp_localize_script( 'jquery.min', 'ajax_select', array(
 ...
 * Add Custom Taxonomies
 */
add_action('init', 'wpshout_book_register_taxonomy');
function wpshout_book_register_taxonomy() {
    $args = array( 
        'hierarchical' => true,
        'label' => 'Books Taxonomy',
    );
    register_taxonomy( 'awesome_book_taxonomy', array( 'book-reviews' ), $args );
}
